<template>
	<div class="page-login">
		...
		<a href="https://cool-js.com" class="copyright"> Copyright © YINHEYIBEI </a>
	</div>
</template>
...
import { login } from '/@/modules/api/user/index';
...
const $md5 = instance?.appContext.config.globalProperties.$md5;
...
async function toLogin() {
	...
	try {
		...
		const data = {
			...
			password: $md5(form.password) as string
		};

		login(data).then(async res => {
			user.setToken({ token: res.data });
			await Promise.all(app.events.hasToken.map(e => e()));
			const UserName=user.info?.nickname
